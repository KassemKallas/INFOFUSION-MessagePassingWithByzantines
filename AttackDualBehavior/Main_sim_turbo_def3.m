...
rho = 0.1; %Modello correlazione: 1-rho = probabilita' che la nuova osservazione sia uguale dalla precedente, when 0.5 no markov model
...
i=0;
for ii= 8:20 % for a set of observation windows
m = ii; %numero osservazioni
sim_param.T = m;    
...
Varsh(i,j) = min(PFAr + 1-PDr);
LLR(i,j) = min(PFAr_LLR+1-PDr_LLR);
...
err_maj(i,j) = mean(ER_MAJ);
err_maj_r(i,j) = mean(ER_MAJ_r);
err_opt(i,j) = mean(ER_OTT_STAT);
err_mp(i,j) = mean(ER(end,:));
err_varsh(i,j) = Varsh(i,j);
err_llr(i,j) = LLR(i,j);
end
end
save('err_maj.mat','err_maj');
save('err_mp.mat','err_mp');
save('err_varsh.mat','err_varsh');
save('err_llr.mat','err_llr');
save('obs_wind.mat','obs_wind');
...
% saveas(gcf,'N_20_rho_01_eps_015_m_15_Pmal_05.fig');
